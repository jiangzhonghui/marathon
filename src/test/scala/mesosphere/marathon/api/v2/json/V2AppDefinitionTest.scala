...
class V2AppDefinitionTest extends MarathonSpec with Matchers {
  ...
  test("Validation") {
    ...
    val app = V2AppDefinition(id = "a b".toRootPath)
    ...
    shouldViolate(
      V2AppDefinition(id = "test".toPath, instances = -3, ports = Seq(9000, 8080, 9000)),
      "ports",
      "Elements must be unique"
    )

    shouldNotViolate(
      V2AppDefinition(id = "test".toPath, ports = Seq(0, 0, 8080)),
      "ports",
      "Elements must be unique"
    )
